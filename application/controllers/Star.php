...
class Star extends CI_Controller {
        public function index($star){
        $star=str_replace("%20"," ",$star);
        $config = array();
        $config["base_url"] = base_url() . "star/".$star;
        $config["total_rows"] = $this->common_model->get_video_by_star_record_count($star);
        $config["per_page"] = 24;
        ...
        $data["all_published_videos"] = $this->common_model->get_video_by_star($config["per_page"], $page, $star);
        ...
        $data['title'] = "Watch ".$star."'s". " movies & TV-Series online";
        $data['page_name']='star';
